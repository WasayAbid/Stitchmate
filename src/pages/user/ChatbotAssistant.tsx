...
const ChatbotAssistant: React.FC = () => {
  ...
  const suggestions = [{
    text: 'What fabric suits traditional wear?',
    icon: '🧵'
  }, {
    text: 'Suggest embroidery patterns',
    icon: '✨'
  }, {
    text: 'How to style a wedding outfit?',
    icon: '👰'
  }, {
    text: 'Find matching accessories',
    icon: '💎'
  }];
      ...
      <div className="relative z-10 space-y-6 p-6">
        ...
        y: 0
          ...
          <div className="flex items-center gap-4">
            <motion.div onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} className="relative rounded-lg shadow-xl">
              <motion.div className="absolute inset-0 rounded-full bg-gradient-to-r from-primary via-rose to-gold blur-md" animate={{
              scale: [1, 1.1, 1],
              opacity: [0.4, 0.6, 0.4]
            }} transition={{
              duration: 2,
          ...
          delay: 0.1
            ...
            <GlassmorphicCard variant="glow" className="h-[650px] flex flex-col p-0 overflow-hidden">
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                <AnimatePresence>
                  {messages.map(message => <ChatMessage key={message.id} role={message.role} content={message.content} timestamp={message.timestamp} showSparkle={message.isPositive} />)}
                </AnimatePresence>
              ...
              {/* Input */}
              <motion.div className="p-4 border-t border-border/30 bg-card/50 backdrop-blur-sm" initial={{
              opacity: 0,
              y: 20
            }} animate={{
              opacity: 1,
              ...
              delay: 0.2
                ...
                handleSend();
                  ...
                  <motion.div className="flex-1 relative">
                    <input type="text" value={input} onChange={e => setInput(e.target.value)} placeholder="Ask Sana anything about fashion..." className="w-full h-14 px-5 pr-12 bg-muted/50 border-2 border-border/50 rounded-2xl focus:border-primary focus:ring-4 focus:ring-primary/20 transition-all outline-none text-base shadow-inner" />
                    <Sparkles className="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground/50" />
                  </motion.div>
                  <motion.button type="submit" disabled={!input.trim() || isTyping} className="h-14 px-6 rounded-2xl bg-gradient-to-r from-primary via-rose to-gold text-primary-foreground font-semibold disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 shadow-lg" whileHover={{
                  scale: 1.02,
          ...
          delay: 0.2
            ...
            <GlassmorphicCard variant="elevated">
              ...
              <div className="space-y-2">
                {suggestions.map((suggestion, index) => <motion.button key={index} onClick={() => handleSuggestionClick(suggestion.text)} className="w-full text-left p-3 rounded-xl bg-muted/30 hover:bg-primary/10 border-2 border-transparent hover:border-primary/30 transition-all text-sm flex items-center gap-2 group" whileHover={{
                x: 4
              }} whileTap={{
                scale: 0.98
              }}>
            ...
            <GlassmorphicCard variant="bordered">
              <div className="text-center">
                <motion.div className="w-16 h-16 mx-auto mb-3 rounded-full bg-gradient-to-br from-primary via-rose to-gold p-0.5" animate={{
                rotate: [0, 5, -5, 0]
              }} transition={{
                duration: 4,
                repeat: Infinity
            ...
            {/* Context Info */}
            {(fabricAnalysis || selectedDesign) && <GlassmorphicCard variant="bordered">
                <h3 className="font-semibold mb-3 flex items-center gap-2">
                  <Sparkles className="w-4 h-4 text-primary" />
                  Your Project
                </h3>
                {fabricAnalysis && <div className="p-3 bg-muted/30 rounded-xl mb-2">
                    <p className="text-xs text-muted-foreground">Fabric</p>
                    <p className="text-sm font-medium">{fabricAnalysis.type}</p>
                  </div>}
                {selectedDesign && <div className="p-3 bg-muted/30 rounded-xl">
